...
void MainWindow::loadStoredConnections()
{
    ...
    while(!assets_file.atEnd())
    {
        ...
        {
            constructNewThread(new_asset->asset_id, new_asset->asset_name, new_asset->first_query);
        }
    }
}
...
void MainWindow::constructNewThread(int asset_id, QString& name, QString& query)
{
    ...
    status_fields_vbox->addLayout(connection->getPanelLayout());
    ...
    QObject::connect(new_asset_thread, &QThread::finished, connection, &QObject::deleteLater);
    QObject::connect(new_asset_thread, &QThread::started, connection, &WssConnection::slotThreadStart);
    new_asset_thread->start();
}

void MainWindow::shutExistingThread(assetQuoteService* sender_service)
{
    status_fields_vbox->removeItem(sender_service->asset_connection->getPanelLayout());
...
void MainWindow::slotCheckboxClicked()
{
    ...
    if(checkbox_sender->isChecked())
    {
        ...
        constructNewThread(sender_service->asset_id, sender_service->asset_name, sender_service->first_query);
    }
    else
    {
        sender_service->asset_status = 0;
        shutExistingThread(sender_service);
    }
}
